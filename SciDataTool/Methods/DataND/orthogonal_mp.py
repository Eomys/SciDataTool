...
def orthogonal_mp(self, Time: Data1D, n_coefs: int=None):
    ...
    # This method should only be used for 2D field, where the undersampling
    # in the direction of time
    nombre_axes = len(self.axes)
    assert nombre_axes == 2, "Dimension error: %d != 2".format(nombre_axes)

    axes_name = [self.axes[i].name for i in range(2)]
